<template>
  <div>
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="absolute bg-white rounded-lg shadow-lg border w-40 sm:w-48"
          :style="dropdownStyle"
          @click.stop
        >
          <button
            class="w-full px-4 py-2 text-left text-sm hover:bg-gray-50 flex items-center"
            @click="handleTagModalClick"
          >
            <svg
...
const toggleMenuDropdown = () => {
  if (!showMenuDropdown.value && dropdownContainer.value) {
    // ドロップダウンボタンの位置を取得
...
// メニュー項目のクリックハンドラー
const handleTagModalClick = () => {
  closeMenuDropdown()
  _emit('open-tag-modal')
}
